...
def _write_performance_profile(results, output_dir: Path, metric: str, *, expected: pd.DataFrame | None = None) -> Path | None:
    profile = performance_profile(results, metric=metric, expected=expected)
    if profile.empty:
        return None
